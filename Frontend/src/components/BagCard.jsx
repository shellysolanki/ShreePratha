...
export default function BagCard({ item, onDelete, isAdmin = false }) {
  const { _id, title, price, images, material, color, size, stock, description } = item;
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`https://shreepratha.onrender.com/bag/${_id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' } });
  ...
  const handleUpdate = async () => {
    try {
      const res = await fetch(`https://shreepratha.onrender.com/bag/${_id}`, {
        method: 'PUT',
    ...
    <>
      <div className="card mb-4 shadow-lg h-100" style={{ 
        ...
        <div className="card-body text-center" style={{ backgroundColor: '#ffffff', color: '#333333' }}>
          <h5 className="card-title" style={{ color: '#8b4513', fontWeight: 'bold', marginBottom: '15px', fontSize: '1.3rem' }}>{title}</h5>
          <p className="card-text mb-2" style={{ color: '#666666' }}>
            <strong style={{ color: '#8b4513' }}>Material:</strong> {material}
          </p>
          <p className="card-text mb-2" style={{ color: '#666666' }}>
            <strong style={{ color: '#8b4513' }}>Color:</strong> {color}
          </p>
          <p className="card-text mb-2" style={{ color: '#666666' }}>
            <strong style={{ color: '#8b4513' }}>Size:</strong> {size}
          </p>
          ...
          {!isAdmin && (
            <button 
              ...
              onMouseEnter={(e) => {
                ...
                e.target.style.boxShadow = '0 8px 25px rgba(139, 69, 19, 0.6)';
              }}
              onMouseLeave={(e) => {
                e.target.style.transform = 'translateY(0)';
                e.target.style.boxShadow = '0 4px 15px rgba(139, 69, 19, 0.4)';
              }}
            >
              👜 Buy Now
            </button>
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            <ProductReviews productId={_id} productType="bag" />
          </div>
        </div>
      )}
