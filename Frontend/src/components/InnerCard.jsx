...
export default function InnerCard({ item, onDelete, isAdmin = false }) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`https://shreepratha.onrender.com/inner/${_id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' } });
  ...
  const handleUpdate = async () => {
    try {
      const res = await fetch(`https://shreepratha.onrender.com/inner/${_id}`, {
        method: 'PUT',
