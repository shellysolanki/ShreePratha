...
export default function JewelCard({ item, onDelete, isAdmin = false }) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`http://localhost:5000/jewellery/${_id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' } });
  ...
  const handleUpdate = async () => {
    try {
      const res = await fetch(`http://localhost:5000/jewellery/${_id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updateForm) });
      const data = await res.json();
      if (res.ok) {
...
//     setDeleting(true);
//     try {
//       const res = await fetch(`http://localhost:5000/jewellery/${_id}`, {
//         method: 'DELETE',
